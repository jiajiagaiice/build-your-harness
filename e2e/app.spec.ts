...
test('validates the default workflow and edits a newly added skill', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '构建你的 Harness' })).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Add Requesting Code Review' }).click();

  const skillEditor = page.getByRole('complementary').filter({ has: page.getByRole('heading', { name: 'Skill Editor' }) });
  await expect(skillEditor.getByLabel('Label')).toHaveValue('Requesting Code Review');
  ...
  await expect(skillEditor.getByLabel('SKILL.md content')).toHaveValue(/# Requesting Code Review/);
});
